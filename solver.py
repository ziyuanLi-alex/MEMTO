...
def _adjust_predicts(score, label, threshold, pred=None,
                     calc_latency=False):
    """
    Adjust predictions using point-adjustment protocol (from Anomaly Transformer).
    Returns adjusted predictions and (optionally) latency info.
    """
    if pred is None:
...
def _compute_range_metrics(pred, gt):
    """
    Compute range-based precision, recall, and F-score.
    
    A range is a contiguous segment of 1s in the label/prediction.
    - Range-Precision: fraction of predicted ranges that overlap at least one true range.
    - Range-Recall: fraction of true ranges that overlap at least one predicted range.
    
    Reference: "Explainable Time Series Anomaly Detection via Temporal Decomposition" 
    and MSL/SMAP benchmark evaluation protocol.
    """
    pred = pred.astype(int)
    gt = gt.astype(int)

    def _get_ranges(arr):
        """Return list of (start, end) for each contiguous range of 1s."""
        ranges = []
        in_range = False
        start = 0
        for i in range(len(arr)):
            if arr[i] == 1 and not in_range:
                in_range = True
                start = i
            elif arr[i] == 0 and in_range:
                in_range = False
    ...
    true_ranges = _get_ranges(gt)
    pred_ranges = _get_ranges(pred)

    if len(true_ranges) == 0:
        return 1.0, 1.0, 1.0  # no anomalies → perfect score

    if len(pred_ranges) == 0:
        return 0.0, 0.0, 0.0  # no predictions → zero score

    # Range recall: how many true ranges were hit by at least one pred range
    true_hit = 0
    for t_start, t_end in true_ranges:
        for p_start, p_end in pred_ranges:
            if p_start <= t_end and p_end >= t_start:  # overlap
                true_hit += 1
                break
    range_recall = true_hit / len(true_ranges)

    # Range precision: how many pred ranges hit at least one true range
    pred_hit = 0
    for p_start, p_end in pred_ranges:
        for t_start, t_end in true_ranges:
            if p_start <= t_end and p_end >= t_start:  # overlap
                pred_hit += 1
                break
    range_precision = pred_hit / len(pred_ranges)
    ...
    return range_precision, range_recall, range_f
...
class EarlyStopping:
    """Early stopping that monitors a single validation loss."""
...
class Solver(object):
    ...
    def train(self, training_type):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        early_stopping = EarlyStopping(patience=10, verbose=True, dataset_name=self.dataset, type=training_type)
        ...
        for epoch in tqdm(range(self.num_epochs)):
            ...
            for i, (input_data, labels) in enumerate(self.train_loader):
                ...
                try:
                    ...
                    raise RuntimeError(f"Backward failed at epoch {epoch+1}, iter {i+1}: {e}")
                self.optimizer.step()
    ...
    def test(self):
        ...
        def _compute_energy(loader):
            """Compute anomaly energy for a given loader."""
            energy_list = []
            for i, (input_data, _) in enumerate(loader):
                ...
                energy_list.append(cri)
            return np.concatenate(energy_list, axis=0).reshape(-1)

        train_energy = _compute_energy(self.train_loader)
        valid_energy = _compute_energy(self.thre_loader)
        ...
        # AUC-PR (using raw energy scores and ground truth)
        auc_pr = average_precision_score(gt, test_energy)

        # Range-based F-score
        range_precision, range_recall, range_f = _compute_range_metrics(pred, gt)

        print("Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f} ".format(
            accuracy, precision, recall, f_score))
        print("AUC-PR : {:0.4f}".format(auc_pr))
        print("Range-Precision : {:0.4f}, Range-Recall : {:0.4f}, Range-F : {:0.4f} ".format(
            range_precision, range_recall, range_f))
        print('=' * 50)
        ...
        self.logger.info(f"AUC-PR: {round(auc_pr, 4)}")
        self.logger.info(f"Range-Precision: {round(range_precision, 4)}")
        self.logger.info(f"Range-Recall: {round(range_recall, 4)}")
        self.logger.info(f"Range-F: {round(range_f, 4)} \n")

        return accuracy, precision, recall, f_score, auc_pr, range_precision, range_recall, range_f
